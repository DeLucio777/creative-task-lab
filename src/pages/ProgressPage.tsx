...
const ProgressPage: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (role === 'parent' && user) {
        // В новой схеме родитель = ребёнок (role=1), видит только собственный прогресс
  ...
  useEffect(() => {
    if (selectedChild) achievementsApi.getByUser(selectedChild).then(setUserAchievements);
  ...
  const achName = (id: number) => achievements.find(a => a.id === id)?.name || `🏅 #${id}`;
  ...
  return (
    <div>
      ...
      {selectedChild > 0 && userAchievements.length > 0 && (
        <div className="mb-6">
          <h3 className="font-bold text-foreground mb-3 flex items-center gap-2"><Star className="h-5 w-5 text-warning" /> Достижения</h3>
          <div className="flex gap-2 flex-wrap">
            {userAchievements.map(r => (
              <div key={r.id} className="bg-card border-2 border-border rounded-xl px-4 py-2 text-center">
                ...
                {r.earned_date && <p className="text-xs text-muted-foreground">{new Date(r.earned_date).toLocaleDateString('ru')}</p>}
              </div>
            ))}
          </div>
        </div>
