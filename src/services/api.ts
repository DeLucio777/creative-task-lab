...
async function apiFetch<T>(endpoint: string, fallback: T): Promise<T> {
    try {
        ...
        console.log(`[API] Fetched from SERVER: ${endpoint}`, data);
        return data;
    } catch {
...
export const api = {
    ...
    createFullTask: async (payload: {
        ...
        try {
            const url = API_BASE ? `${API_BASE}/api/tasks/full` : '/api/tasks/full';
            const res = await fetch(url, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
    ...
    createTask: async (task: Partial<Task>): Promise<Task | null> => {
        const url = API_BASE ? `${API_BASE}/api/tasks` : '/api/tasks';
    ...
    uploadPecs: async (file: File, description: string, category: string): Promise<CatalogPECS | null> => {
        const url1 = API_BASE ? `${API_BASE}/api/pecs` : '/api/pecs';
        ...
        try {
            ...
            const res = await fetch(url1, {method: 'POST', body: fd});
            if (res.ok) return await res.json();
            throw new Error(`HTTP ${res.status}`);
        } catch {
    ...
    uploadMedia: async (file: File, description: string): Promise<MediaCatalog | null> => {
        const url1 = API_BASE ? `${API_BASE}/api/media` : '/api/media';
    ...
    deleteTask: async (taskId: number): Promise<boolean> => {
        const url1 = API_BASE ? `${API_BASE}/api/tasks/${taskId}` : `/api/tasks/${taskId}`;

        try {
            const res = await fetch(url1, {method: 'DELETE'});
