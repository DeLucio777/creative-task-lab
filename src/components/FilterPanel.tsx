...
const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 bg-card rounded-xl border border-border p-5 z-50 shadow-[0_12px_24px_rgba(0,0,0,0.08)]">
          <div className="flex justify-between items-center mb-4">
            <h4 className="font-semibold text-sm text-foreground">Фильтры</h4>
          ...
          <div className="space-y-4">
            ...
            <div>
              <p className="text-xs font-medium text-muted-foreground mb-2 uppercase tracking-wider">Сложность</p>
            ...
            <div>
              <p className="text-xs font-medium text-muted-foreground mb-2 uppercase tracking-wider">Типы РАС</p>
              <div className="space-y-2">
                {RAS_TYPES.map(r => (
                  <div key={r.value} className="flex items-center gap-2">
                    ...
                    />
                    <Label htmlFor={`ras-${r.value}`} className="text-sm cursor-pointer">{r.label}</Label>
                  </div>
                ))}
              </div>
            </div>
